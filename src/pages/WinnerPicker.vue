<template>
  <q-page class="flex flex-center bg-black">
    ...
    <q-card v-if="!showWinnerScreen" class="add-names column bg-red">
      <q-card-section>
        <h2 class="text-center q-mt-none q-mb-md text-white text-bold deep-shadow">SORTEAR PESSOA</h2>
        <q-input
          v-model="namesText"
          filled
          type="textarea"
          bg-color="white"
          placeholder="Coloque o nome em linhas separadas"
          rows="10"
          cols="40"
        />
    ...
    <div v-else class="winner-screen">
      <h1 v-if="pickedWinner" class="text-white absolute-top text-center text-bold deep-shadow">VENCEDOR!!!</h1>
      <div :class="{'zoom' : zoomingNames}" class="name-card">
        <q-card
          v-if="currentName"
          dark
    ...
    <audio v-if="showWinnerScreen" src="/statics/drumroll.mp3" autoplay></audio>
  </q-page>
</template>
